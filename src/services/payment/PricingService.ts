...
import { PAYMENT_PLANS, formatPrice } from '../../lib/stripe-config.ts';
  ...
  getPricingInfo(role: 'huurder' | 'verhuurder') {
    if (role === 'huurder') {
      ...
      return {
        ...
        description: `${formatPrice(plan.price)} eenmalige betaling (BTW inbegrepen)`
      };
    } else {
      const plan = PAYMENT_PLANS.verhuurder.free;
      return {
        displayPrice: 'Gratis',
        actualPrice: 'Gratis',
