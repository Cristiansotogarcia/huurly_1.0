...
--  This block safely creates types without erroring if they exist.
-- =================================================================
...
--  Using DROP...CREATE makes this section re-runnable.
-- =================================================================

-- Policies for woningen
DROP POLICY IF EXISTS "Verhuurders kunnen eigen woningen beheren" ON public.woningen;
CREATE POLICY "Verhuurders kunnen eigen woningen beheren" ON public.woningen
  FOR ALL USING (
    EXISTS (SELECT 1 FROM public.gebruikers g WHERE g.id = auth.uid() AND g.rol = 'verhuurder' AND g.id = verhuurder_id)
  ) WITH CHECK (
    EXISTS (SELECT 1 FROM public.gebruikers g WHERE g.id = auth.uid() AND g.rol = 'verhuurder' AND g.id = verhuurder_id)
  );

DROP POLICY IF EXISTS "Huurders kunnen actieve woningen bekijken" ON public.woningen;
CREATE POLICY "Huurders kunnen actieve woningen bekijken" ON public.woningen
  FOR SELECT USING (
    is_actief = true
    AND status = 'actief'
    AND EXISTS (SELECT 1 FROM public.gebruikers g WHERE g.id = auth.uid() AND g.rol = 'huurder')
  );

-- Policies for aanvragen
DROP POLICY IF EXISTS "Eigen aanvragen beheren" ON public.aanvragen;
CREATE POLICY "Eigen aanvragen beheren" ON public.aanvragen
  FOR ALL USING (
    auth.uid() = huurder_id
    OR auth.uid() = verhuurder_id
    OR EXISTS (SELECT 1 FROM public.gebruiker_rollen gr WHERE gr.user_id = auth.uid() AND gr.role IN ('admin'))
  ) WITH CHECK (
    auth.uid() = huurder_id
    OR auth.uid() = verhuurder_id
  );

-- Policies for berichten
DROP POLICY IF EXISTS "Eigen berichten beheren" ON public.berichten;
...
--  Using DROP...ADD makes this section re-runnable.
-- =================================================================

ALTER TABLE public.berichten DROP CONSTRAINT IF EXISTS berichten_verzender_id_fkey;
ALTER TABLE public.berichten ADD CONSTRAINT berichten_verzender_id_fkey
  FOREIGN KEY (verzender_id) REFERENCES public.gebruikers(id) ON DELETE CASCADE;

ALTER TABLE public.berichten DROP CONSTRAINT IF EXISTS berichten_ontvanger_id_fkey;
ALTER TABLE public.berichten ADD CONSTRAINT berichten_ontvanger_id_fkey
  FOREIGN KEY (ontvanger_id) REFERENCES public.gebruikers(id) ON DELETE CASCADE;


-- =================================================================
--  7. DEFINE FUNCTIONS AND TRIGGERS
--  Using CREATE OR REPLACE and DROP...CREATE makes this re-runnable.
-- =================================================================
...
DROP TRIGGER IF EXISTS aanvragen_updated_at ON public.aanvragen;
